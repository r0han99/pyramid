...
try:
    if sys.argv[1] == '--set':
        ...
        elif(os.stat(str(os.getenv("HOME"))+'/Documents/.projectstackdb/projectstack.csv').st_size == 31 and len(sys.argv) == 2):
            ...
            while True:
                ...
                if(SET_OPTION == 'y'):
                    ...
                    for path in paths:
                        ...
                        if re.search(r"\s",path.strip()):
                            print(color.RED+str(flag)+' '+path.strip()+" - "+color.END+"[["+color.WARNING+"ignored"+color.END+"] unconventional naming, contains spaces]")
                            print()
                            init_prior = 0
                            flag+=1
                            continue
    ...
    providing the pathfile as an arugument '"""+color.GREEN+"""--set <pathfile.txt> """+color.WARNING+"""or during the run-time.
        ...
        elif(os.stat(str(os.getenv("HOME"))+'/Documents/.projectstackdb/projectstack.csv').st_size == 31 and len(sys.argv) == 3 ):

            print('['+color.GREEN+'Parsing the file with paths.'+color.END+']')
            if( len(sys.argv) == 3):
